...
class NodeState:
    ...
    def getAttributesSignature(self):
        ...
        if self._attributesSignature == None:
            self._updateSignature()
...
class DirectoryNodeState(NodeState):
    ...
    def _appendToWalkPath(self, walkPath):
        ...
        for child in self._children:
            child._appendToWalkPath(walkPath)
    ...
    def getChildren(self, types=None):
        ...
        if types == None:
            types = (DirectoryNodeState, FileNodeState)
...
class State:
    ...
    def FromJSONFile(path):
        ...
        with open(path, "r") as f:
            return State.FromDict(json.load(f))
    ...
    def __init__(self, rootLocation, rootNodeState=None, populate=False,
        ...
        if rootLocation:
            self.location(os.path.abspath(rootLocation))
        else:
            self.location(None)
    ...
    def rejects(self, a):
        ...
        added."""
        if type(a) in (tuple, list):
            self._rejects.extend(a)
        else:
            self._rejects.append(a)
    ...
    def root(self, node=None):
        ...
        if node != None:
            self._rootNodeState = node
        else:
            return self._rootNodeState
...
class Engine:
    ...
    def run(self, arguments):
        ...
        if not os.path.exists(origin_path):
            logger.error("Origin directory does not exist.")
            return -1
        for path in compared_paths:
            if not os.path.exists(path):
                logger.error("Compared directory does not exist.")
                return -1
    ...
    def listChanges(self, changes, origin, compared, diffs=[], diffcommand="diff", show=None):
        ...
        if not all_locations_keys:
            self.logger.message("No changes found.")
